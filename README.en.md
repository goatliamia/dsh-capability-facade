...
## Tools (the first two steps of the refactoring protocol)

The repository also carries the first two steps of the plugin-refactoring protocol
(design: [`docs/refactor-protocol-review.md`](docs/refactor-protocol-review.md)).

### 1. `analyze` — read-only static analysis
...
node scripts/analyze-plugin.mjs <plugin-dir> --propose    # + a starter declaration
```

It reports which model-facing tools a plugin registers, groups them by name prefix, marks the
ones that *look like* implementation primitives, and asks a **question** (never a
recommendation). It is read-only: a test proves it changes not one byte of the directory.

It does not hide, move, or rewrite a tool, and never claims one is unnecessary. Grouping and
classification are heuristics; "are these two tools one operation?" is a model/human judgment.

### 2. `verify` — isolated profile verification

```powershell
.\scripts\verify-plugin.ps1 -PluginDir <dir> -PluginName my-plugin           # compose check
.\scripts\verify-plugin.ps1 -PluginDir <dir> -PluginName my-plugin -Smoke    # real boot
```

It composes `dsh-base + dsh-headless + your plugin` in a throwaway profile
(`verify-<random>`), checks the composed tree, and with `-Smoke` boots it once and expects the
agent to answer `VERIFY-OK`. It never touches your target profile and deletes the throwaway
profile afterwards.

Why boot at all: `--dump-config` validates composition but activates nothing, and a load-time
throw takes down the **whole** plugin tree (community discussions #5106 / #5237 are exactly
this). Measured: a plugin that deliberately throws passes the compose stage and is caught at
the boot stage.
...
| `lib/analyze.mjs` | The read-only static analyzer. |
...
| `test/analyze.test.mjs` | Analyzer tests, including the "writes nothing" proof. |
| `test/loader.mjs` | Test-only resolution shim for this workspace (not shipped). |
| `scripts/analyze-plugin.mjs` | CLI for `analyze`. |
| `scripts/verify-plugin.ps1` | Isolated verification for `verify`. |
...
npm test                                    # 49 assertions → OK
node test/analyze.test.mjs                  # 19 assertions → OK
