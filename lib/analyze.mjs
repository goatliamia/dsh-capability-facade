/**
 * Static analyzer for DSH plugin tool surfaces — the read-only first step of the
 * plugin-refactoring protocol (`docs/refactor-protocol-review.md`).
 *
 * It answers one question about a plugin directory:
 *
 *   > Which model-facing tools does this plugin register, which of them look
 *   > like implementation primitives, and what would a capability declaration
 *   > over them look like?
 *
...
/**
 * Find every statically visible `defineTool({...})` call in one source file.
 * A name containing `${…}` or a concatenation is reported as dynamic, never
 * guessed — the analyzer does not invent identifiers.
 * @returns {Array<object>} tool records.
 */
function findToolDefinitions(text, file) {
  const found = []
  const pattern = /(?:harness\s*\.\s*)?defineTool\s*\(\s*\{/g
  let match
  while ((match = pattern.exec(text)) !== null) {
    const start = text.indexOf('{', match.index)
    const objectText = balancedObject(text, start)
    if (objectText === undefined) continue
    const rawName = stringProperty(objectText, 'name')
